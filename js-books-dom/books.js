...
/**Ajout des boutons cacher/montrer
 * Ajouter juste avant le titre h1 la possibilité de masquer ou montrer les descriptions courtes
(shortDescription). On pourra définir un élément div qui contient deux éléments span, l’un
pour cacher, l’autre pour montrer. On utilisera la classe cache pour laquelle le fichier
...
function boutons(){
	...
	const div = document.createElement('div');
	const buttonmontrer = document.createElement('button');
	const buttoncacher = document.createElement('button');
	const namemontrer = document.createAttribute("name")
	namemontrer.value = "montrer"
	const idmontrer = document.createAttribute("id")
	idmontrer.value = "montrer";
	const namecacher = document.createAttribute("name")
	namecacher.value = "cacher";
	const idcacher = document.createAttribute("id")
	idcacher.value = "cacher";
	
	div.appendChild(buttonmontrer);
	buttonmontrer.setAttributeNode(namemontrer);
	buttonmontrer.setAttributeNode(idmontrer);
	buttonmontrer.innerText = 'montrer';

	div.appendChild(buttoncacher);
	buttoncacher.setAttributeNode(namecacher);
	buttoncacher.setAttributeNode(idcacher);
	buttoncacher.innerText = 'cacher';

	

	//prendre toutes les dt...
	const alldt = document.querySelectorAll('dt'); 

	console.log('alldt', alldt)

	//...dont la valeur est shortDescription
	alldt.forEach(dt =>{ 						//itération sur toutes les dts
		if (dt.innerText === 'shortDescription'){
			const span = document.createElement('span');
			
			
			const dd = dt.nextElementSibling;
			span.textContent = dd.innerText;// le contenu de span prend le contenu de dd
			dd.textContent = ''; // vide le contenu de dd
			dd.appendChild(span); //on intègre la balise span dans le dom
		} 


		//pour faire montrer cacher, attribuer la class cache à tous les éléments  DT DD de DL qu'on veut. et pas insérer le span
...
buttoncacher.addEventListener('click', event => { 
...
document.body.insertAdjacentElement('afterbegin', boutons())
